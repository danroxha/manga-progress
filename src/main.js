...
function getMangaData(){
  ...
  try {
    ...
    manga['title'] = String($$('.title')[0]?.textContent ?? $('.html-embed-3')?.children[0]?.children[0]?.textContent?.match(/(\w+.*[-])/)[0]?.slice(0, -2)) 
	  manga['cover'] = String(document.querySelector('.image-3')?.src ?? '') 
    manga['address'] = String(location.href.match(/http?s:\/\/\w*.\w+\/\w+\/\w+?.+[^(\/\d+.\d+)#]/)[0]) 
    manga['address-page'] =  String('')
	  manga['id'] = Number(location.pathname.match(/\d+/)[0])
    manga['page'] = Number((location.hash.length) ? location.hash.replace('#', '') : 0)
...
async function main(){
  ...
  try {
    ...
    chrome.storage.sync.set({'currentManga': manga }, function() {
      console.log(`current manga found`)
    })
